		...
		<div class="editaddr-cnt">
			<form class="editaddr-form">
				<p class="form-ctrl tb1px">
					...
					<input v-model="addrInfo.contact_name" maxlength="5" type="text" class="name" id="name" placeholder="请输入收货人真实姓名">
				</p>
				<p class="form-ctrl tb1px">
...
<script>
	...
	export default {
		...
		methods:{
			...
			dealSuccessResult(){
				...
				if (this.isDefaulEdit) {					
					// let beforePath = '',
					// 	beforeQuery = {};
					// //如果beforeRouteEnter钩子 拿到了正确的 from 对象					
					// if (this.beforeRoutePath && this.beforeRoutePath !== '/') {
					// 	beforePath = this.beforeRoutePath;
					// 	beforeQuery = this.beforeRouteQuery;						
					// } else {
					// 	let path = this.$route.path,
					// 		query = this.$route.query;					
					// 	beforePath = path.split('/').slice(0, -1).join('/'),
					// 	beforeQuery = JSON.parse(JSON.stringify(query));
					// 	delete beforeQuery.eidx;
					// }	
					// this.redirect(beforePath, beforeQuery);		
		...
		// 	next(vm => {
		// 		vm.beforeRoutePath = from.path;
		// 		vm.beforeRouteQuery = from.query;
		// 	});
		// }
	}
</script>
