<template>
	<div class="newaddr-wrapper">
		<div class="newaddr-cnt"  v-show="pageLoaded">
			<form class="newaddr-form">
				<p class="form-ctrl tb1px">
					<label class="lbl" for="name">收货人</label>
					<input v-model="formData.name" type="text" maxlength="5" class="name" id="name" placeholder="请输入收货人真实姓名">
				</p>
				<p class="form-ctrl tb1px">
				...
				<p class="form-ctrl default" v-if="isDefaultEdit && isShowSetDef && addrList.length>0">
					<span class="lbl">设为默认</span>
					<span class="switch">
						<input v-model="formData.isDefault" type="checkbox" class="switch-inp" id="defaultSetRadio">
						<label class="switch-lbl tb1px" for="defaultSetRadio">
						</label>
					</span>
				</p>
			</form>
...
<script>
	...
	export default {
		...
		methods:{
			...
			smbtAddrData(){
				if (this.isSubmiting || !this.checkFormData()) {
					return;
				}
			...
			onValuesChange(picker, values) {
				...
				// 	const districtData =  [this.addrPre[2]].concat(Object.keys(CityData[values[0]][values[1]]));
				// 	districtData && picker.setSlotValues(2,districtData);
				// 	this.lastCity = city;
				// }
		...
		mounted(){
			...
			const showDefReg = /profile/g;
			this.isDefaultEdit = !re.test(this.$route.path);
			this.isShowSetDef = showDefReg.test(this.$route.path);
			setTimeout(()=>{
...
<style scoped>
	...
	.newaddr-form{
		.form-ctrl{
			...
			&:nth-last-child(2).area,&:nth-last-child(1){
